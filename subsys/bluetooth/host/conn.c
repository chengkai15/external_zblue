...
int bt_conn_le_subrate_set_defaults_mc(uint8_t dev_id, const struct bt_conn_le_subrate_param *params)
{
...
{
	return bt_conn_le_subrate_set_defaults_mc(0, params);
}
...
{
	if (param->conn_interval_min < BT_HCI_LE_SCI_CONN_INTERVAL_MIN ||
	    param->conn_interval_min > BT_HCI_LE_SCI_CONN_INTERVAL_MAX ||
	    param->conn_interval_max < BT_HCI_LE_SCI_CONN_INTERVAL_MIN ||
	    param->conn_interval_max > BT_HCI_LE_SCI_CONN_INTERVAL_MAX ||
...
int bt_conn_le_conn_rate_set_defaults(uint8_t dev_id, const struct bt_conn_le_conn_rate_param *params)
{
	...
	hdev = bt_dev_get(dev_id);
	if (!hdev || !BT_CMD_TEST(hdev->supported_commands, 48, 6)) {
		return -ENOTSUP;
	}
				 ...
				 const struct bt_conn_le_conn_rate_param *params)
{
	...
	if (!BT_CMD_TEST(conn->hdev->supported_commands, 48, 5)) {
		return -ENOTSUP;
	}
...
int bt_conn_le_read_min_conn_interval_groups(uint8_t dev_id, struct bt_conn_le_min_conn_interval_info *info)
{
	...
	hdev = bt_dev_get(dev_id);
	if (!hdev || !BT_CMD_TEST(hdev->supported_commands, 48, 7)) {
		return -ENOTSUP;
	}
