...
LOG_MODULE_REGISTER(hogp_dev, LOG_LEVEL_DBG);

/* HOGP v1.1 §7.1: "HID Service characteristics shall require an encrypted link" */
...
/* HIDS SCI CR §2.X / HOGP v1.1 Table 6.3: Security Permissions = None */
#define HOGP_EXT_PERM_READ   (BT_GATT_PERM_READ)
#define HOGP_EXT_PERM_WRITE  (BT_GATT_PERM_WRITE)
#define HOGP_EXT_PERM_RW     (HOGP_EXT_PERM_READ | HOGP_EXT_PERM_WRITE)
...
static const struct bt_conn_le_conn_rate_param sci_mode_params[SCI_MODE_NUM_PRESETS] = {
	/* DEFAULT (0x02): Spec HOGP_HID_SCI_CR Table X.4 */
	[0] = {.conn_interval_min = 60,   /* 7.5 ms */
	       .conn_interval_max = 120,  /* 15 ms */
	       .max_latency = 0,
	       .subrate_min = 1,
	       .subrate_max = 4,
	       ...
	       .min_ce_length = 1,
	       .max_ce_length = 1},
	/* FAST (0x03): Spec HOGP_HID_SCI_CR Table X.4 */
	[1] = {.conn_interval_min = 10,   /* 1.25 ms */
	       .conn_interval_max = 40,   /* 5 ms */
	       .max_latency = 0,
	       .subrate_min = 1,
	       .subrate_max = 4,
	       ...
	       .min_ce_length = 0,
	       .max_ce_length = 0},
	/* LOW_POWER (0x04): Spec HOGP_HID_SCI_CR Table X.4 */
	[2] = {.conn_interval_min = 60,   /* 7.5 ms */
	       .conn_interval_max = 120,  /* 15 ms */
	       .max_latency = 100,
	       .subrate_min = 1,
	       .subrate_max = 4,
	       ...
	       .min_ce_length = 1,
	       .max_ce_length = 1},
	/* FULL_RANGE (0x05): Spec HOGP_HID_SCI_CR Table X.4 */
	[3] = {.conn_interval_min = 10,   /* 1.25 ms = FAST min */
	       .conn_interval_max = 120,  /* 15 ms */
	       .max_latency = 0,
	       .subrate_min = 1,
	       .subrate_max = 4,
...
{
	switch (mode) {
	case BT_HOGP_SCI_MODE_DEFAULT:
		return 0;
	case BT_HOGP_SCI_MODE_FAST:
		return 1;
	case BT_HOGP_SCI_MODE_LOW_POWER:
		return 2;
	case BT_HOGP_SCI_MODE_FULL_RANGE:
		return 3;
	default:
		return -1;
	}
...
#define ISO_SDU_MAX_SIZE BT_HOGP_ISO_DEFAULT_SDU_SIZE
...
static void iso_connected_cb(struct bt_iso_chan *chan)
{
	...
	/* Get ISO interval for TX pacing */
	if (bt_iso_chan_get_info(chan, &info) == 0) {
		/* iso_interval is in units of 1.25ms, convert to us */
		dc->iso_tx_interval_us = BT_GAP_ISO_INTERVAL_TO_US(info.iso_interval);
	} else {
		dc->iso_tx_interval_us = 1000U; /* fallback: 1 ms */
	}
			...
			struct net_buf *buf)
{
	...
	if (callbacks && callbacks->set_report) {
		callbacks->set_report(dc->conn, BT_HID_REPORT_TYPE_OUTPUT, report_id,
				      buf->data, length);
	}
...
static int iso_accept_cb(const struct bt_iso_accept_info *info, struct bt_iso_chan **chan)
{
	...
	if (info->acl != dc->conn) {
		LOG_WRN("ISO accept: ACL mismatch, rejecting CIS from wrong peer");
		return -EACCES;
	}

	dc->iso_tx_qos.sdu = iso_svc.max_sdu_input;
	dc->iso_rx_qos.sdu = iso_svc.max_sdu_output;
	dc->iso_qos.tx = &dc->iso_tx_qos;
	dc->iso_qos.rx = &dc->iso_rx_qos;

	dc->iso_chan.ops = &iso_chan_ops;
	dc->iso_chan.qos = &dc->iso_qos;
	*chan = &dc->iso_chan;
	return 0;
}
			   ...
			   const uint8_t *data, uint16_t len)
{
	...
	if (len + BT_HOGP_ISO_PKT_HDR_SIZE > ISO_SDU_MAX_SIZE) {
		LOG_WRN("ISO report too large: %u + %u > %u",
			len, BT_HOGP_ISO_PKT_HDR_SIZE, ISO_SDU_MAX_SIZE);
		return -EMSGSIZE;
	}
				 ...
				 const void *buf, uint16_t len, uint16_t offset, uint8_t flags)
{
	...
	if (opcode == BT_HOGP_ISO_OPCODE_SELECT_HYBRID) {
	...
	} else if (opcode == BT_HOGP_ISO_OPCODE_SELECT_DEFAULT) {
...
static int build_hid_iso_attrs(void)
{
	...
	attrs[idx++] = (struct bt_gatt_attr)BT_GATT_ATTRIBUTE(
		...
		&iso_svc.props_chrc);
	attrs[idx++] = (struct bt_gatt_attr)BT_GATT_ATTRIBUTE(
		&uuid_hid_iso_props.uuid, HOGP_EXT_PERM_READ, read_iso_properties, NULL, NULL);
	...
	attrs[idx++] = (struct bt_gatt_attr)BT_GATT_ATTRIBUTE(
		&uuid_hid_iso_op_mode.uuid, HOGP_EXT_PERM_WRITE, NULL, write_iso_op_mode, NULL);
			     ...
			     uint16_t len, uint16_t offset)
{
	...
	uint8_t mode = dc ? dc->sci_mode : BT_HOGP_SCI_MODE_NONE;
				  ...
				  const struct bt_conn_le_conn_rate_changed *params)
{
	...
	if (params->status != 0) {
		LOG_ERR("SCI conn rate change failed: 0x%02x, "
			...
			params->status);
		/*
		 * 0x20A1 accepted but Connection Rate Change event returns
		 * error (e.g. 0x1A Unsupported Remote Feature). Fall back
		 * to standard connection parameter update.
		 */
		...
		if (idx >= 0) {
			...
			dc->sci_pending_mode = 0;
			bt_gatt_notify(conn, sci_ctx.mode_val_attr,
				       &dc->sci_mode,
				       sizeof(dc->sci_mode));
			dc->current_mode =
	...
	bt_gatt_notify(conn, sci_ctx.mode_val_attr, &dc->sci_mode, sizeof(dc->sci_mode));

	dc->current_mode = (dc->current_mode & BT_HOGP_MODE_ISO) |
			   (dc->sci_mode != BT_HOGP_SCI_MODE_NONE ? BT_HOGP_MODE_SCI : 0);
	if (callbacks && callbacks->mode_changed) {
		callbacks->mode_changed(conn, dc->current_mode);
	}
}
...
#if defined(CONFIG_BT_SHORTER_CONNECTION_INTERVALS)
	if (*val >= BT_HOGP_SCI_MODE_DEFAULT && *val <= BT_HOGP_SCI_MODE_FULL_RANGE) {
		int idx;
		...
		if (*val == BT_HOGP_SCI_MODE_LOW_POWER &&
		...
		if (err) {
			...
			/*
			 * 0x20A1 failed. Fall back to standard LE Connection
			 * Parameter Update.
			 *
			 * Unit conversion: SCI intervals are in 0.125ms
			 * units, bt_le_conn_param uses 1.25ms units.
			    ...
			    uint16_t len, uint16_t offset, uint8_t flags)
{
	...
	LOG_DBG("write report (len=%u offset=%u flags=0x%02x)", len, offset, flags);

	if (offset != 0) {
		/* Reports do not support Prepare/Long Write */
...
static void report_ccc_changed(const struct bt_gatt_attr *attr, uint16_t value)
{
	...
	LOG_DBG("report_ccc_changed: idx=%d value=0x%04x attr_handle=0x%04x",
		idx, value, attr->handle);

	if (idx < 0 || idx >= hid_svc.reports_cnt) {
		LOG_WRN("CCC idx %d out of range (reports_cnt=%d)",
	...
	if (value & BT_GATT_CCC_NOTIFY) {
		...
		LOG_DBG("report[%d] notify ENABLED, ntf_mask=0x%x",
			idx, hid_svc.input_ntf_enabled);
	} else {
		hid_svc.input_ntf_enabled &= ~BIT(idx);
		LOG_DBG("report[%d] notify DISABLED, ntf_mask=0x%x",
			idx, hid_svc.input_ntf_enabled);
	}
}
...
static int build_hids_attrs(void)
{
	...
	hid_svc.attrs[idx++] = (struct bt_gatt_attr)BT_GATT_ATTRIBUTE(
		...
		&prot_mode_chrc);
	hid_svc.attrs[idx++] = (struct bt_gatt_attr)BT_GATT_ATTRIBUTE(
		&uuid_protocol_mode.uuid, HIDS_PERM_RW, read_protocol_mode,
	...
	hid_svc.attrs[idx++] = (struct bt_gatt_attr)BT_GATT_ATTRIBUTE(
		...
		&report_map_chrc);
	hid_svc.attrs[idx++] = (struct bt_gatt_attr)BT_GATT_ATTRIBUTE(
		&uuid_report_map.uuid, HIDS_PERM_READ, read_report_map, NULL, NULL);
	...
	for (i = 0; i < hid_svc.reports_cnt; i++) {
		...
		hid_svc.attrs[idx++] = (struct bt_gatt_attr)BT_GATT_ATTRIBUTE(
			...
			&report_chrcs[i]);
		hid_svc.attrs[idx++] = (struct bt_gatt_attr)BT_GATT_ATTRIBUTE(
			&uuid_report.uuid, HIDS_PERM_RW, read_report, write_report,
		...
		hid_svc.attrs[idx++] = (struct bt_gatt_attr)BT_GATT_ATTRIBUTE(
			&report_ref_uuid.uuid, HIDS_PERM_READ, read_report_ref, NULL,
	...
	hid_svc.attrs[idx++] = (struct bt_gatt_attr)BT_GATT_ATTRIBUTE(
		...
		&hid_info_chrc);
	hid_svc.attrs[idx++] = (struct bt_gatt_attr)BT_GATT_ATTRIBUTE(
		&uuid_hids_info.uuid, HIDS_PERM_READ, read_hid_info, NULL, NULL);
	...
	hid_svc.attrs[idx++] = (struct bt_gatt_attr)BT_GATT_ATTRIBUTE(
		...
		&ctrl_point_chrc);
	hid_svc.attrs[idx++] = (struct bt_gatt_attr)BT_GATT_ATTRIBUTE(
		&uuid_ctrl_point.uuid, HIDS_PERM_WRITE, NULL, write_ctrl_point, NULL);
	...
	if (sci_ctx.modes & HOGP_DEVICE_SCI_MODE_SUPPORTED) {
		...
		hid_svc.attrs[idx++] = (struct bt_gatt_attr)BT_GATT_ATTRIBUTE(
			...
			&sci_info_chrc);
		hid_svc.attrs[idx++] = (struct bt_gatt_attr)BT_GATT_ATTRIBUTE(
			&uuid_sci_info.uuid, HOGP_EXT_PERM_READ, read_sci_info, NULL, NULL);
		...
		hid_svc.attrs[idx++] = (struct bt_gatt_attr)BT_GATT_ATTRIBUTE(
			...
			&sci_mode_chrc);
		hid_svc.attrs[idx++] = (struct bt_gatt_attr)BT_GATT_ATTRIBUTE(
			&uuid_sci_mode.uuid, HOGP_EXT_PERM_READ, read_sci_mode, NULL, NULL);
		sci_ctx.mode_val_attr = &hid_svc.attrs[idx - 1];
		hid_svc.attrs[idx++] = (struct bt_gatt_attr)BT_GATT_ATTRIBUTE(
			&uuid_gatt_ccc.uuid,
			HOGP_EXT_PERM_READ | HOGP_EXT_PERM_WRITE,
			bt_gatt_attr_read_ccc, bt_gatt_attr_write_ccc,
			&sci_ctx.mode_ccc);
	}
#endif /* CONFIG_BT_SHORTER_CONNECTION_INTERVALS */
	...
	LOG_DBG("build_hids_attrs done: %u attrs built", idx);
	return 0;
}
...
int bt_hogp_device_register(const struct bt_hogp_device_init_param *param)
{
	...
	if (sci_ctx.modes & HOGP_DEVICE_SCI_MODE_SUPPORTED) {
		...
		hid_svc.hid_info[HID_INFO_FLAGS_OFFSET] |= BT_HOGP_FLAG_SCI_SUPPORTED;
		if (sci_ctx.modes & HOGP_DEVICE_SCI_MODE_LP_SUPPORTED) {
			hid_svc.hid_info[HID_INFO_FLAGS_OFFSET] |=
				BT_HOGP_FLAG_SCI_LP_SUPPORTED;
		}

		/*
		 * SCI Info characteristic payload comes from the Controller
		 * via LE Read Min Supported Connection Interval. If the
		 * command is unsupported or fails, the characteristic is left
		 * empty; peers can still negotiate via LE Connection Rate
		 * Request.
		 *
		 * Wire format (little-endian):
		 *   uint16_t min_conn_interval (125us units)
		 *   uint8_t  num_groups
		 *   group[i] { uint16_t min; uint16_t max; uint16_t stride; }
		 */
		{
			struct bt_conn_le_min_conn_interval_info info;
			int sci_err;

			sci_err = bt_conn_le_read_min_conn_interval_groups(0, &info);
			if (sci_err == 0) {
				uint8_t *p = sci_ctx.properties;
				uint8_t *end = sci_ctx.properties +
					       sizeof(sci_ctx.properties);
				uint8_t g;

				sys_put_le16(info.min_conn_interval, p);
				p += 2;
				*p++ = info.num_groups;

				for (g = 0; g < info.num_groups &&
					    p + 6 <= end; g++) {
					sys_put_le16(info.groups[g].min_interval, p);
					p += 2;
					sys_put_le16(info.groups[g].max_interval, p);
					p += 2;
					sys_put_le16(info.groups[g].stride, p);
					p += 2;
				}

				sci_ctx.properties_len =
					(uint16_t)(p - sci_ctx.properties);
				LOG_DBG("SCI Info: %u bytes from controller",
					sci_ctx.properties_len);
			} else {
				LOG_WRN("LE Read Min Supported Connection "
					"Interval failed (%d), SCI Info "
					"characteristic will be empty", sci_err);
				sci_ctx.properties_len = 0;
			}
		}
	}
#endif /* CONFIG_BT_SHORTER_CONNECTION_INTERVALS */
	...
	LOG_DBG("build_hids_attrs: attr_count=%u", hid_svc.attr_count);

	hid_svc.svc.attrs = hid_svc.attrs;
	hid_svc.svc.attr_count = hid_svc.attr_count;

	LOG_DBG("HIDS svc before register: attrs=%p count=%u",
	...
	LOG_DBG("HIDS registered OK: first_handle=0x%04x count=%u",
	...
	if (param->iso_enabled) {
		...
		LOG_DBG("HID ISO Service registered, props_len=%u",
			iso_svc.properties_len);

		/* Register ISO server to accept CIS from Host. Per-connection
		 * QoS is initialized in iso_accept_cb() using the SDU sizes
		 * saved here.
		 */
		iso_svc.max_sdu_input = param->iso_max_sdu_input;
		iso_svc.max_sdu_output = param->iso_max_sdu_output;

		iso_svc.server.accept = iso_accept_cb;
		err = bt_iso_server_register(&iso_svc.server);
		if (err) {
			LOG_ERR("Failed to register ISO server: %d", err);
			goto err_unreg_iso_svc;
		}
	}
#endif
...
void bt_hogp_device_unregister(void)
{
	...
	/* 1. Stop app-facing callbacks first. Any in-flight GATT/ISO
	 *    handler racing with us will see callbacks == NULL and skip.
	 *    The module goes silent from this point until re-registered.
	 */
	callbacks = NULL;

	/* 2. Actively disconnect any CIS/ACL still up so the peer knows we
	 *    are going away and the stack finishes its teardown path.
	 */
	for (int i = 0; i < HOGP_DEVICE_MAX_CONNECTIONS; i++) {
		struct hogp_device_conn *dc = &connections[i];
...
#if defined(CONFIG_BT_HOGP_DEVICE_ISO)
		if (dc->iso_cis_connected) {
			bt_iso_chan_disconnect(&dc->iso_chan);
		}
#endif
	...
	/* 3. Drop GATT services. Pending ATT operations targeting these
	 *    attributes will now fail with an attribute-not-found error.
	 */
#if defined(CONFIG_BT_HOGP_DEVICE_ISO)
	...
	/* 4. Release per-connection references and clear module state. */
	for (int i = 0; i < HOGP_DEVICE_MAX_CONNECTIONS; i++) {
			       ...
			       uint16_t len)
{
	...
	if (dc && dc->iso_op_mode == ISO_OP_MODE_HYBRID && dc->iso_cis_connected) {
		return iso_send_report(dc, report_id, BT_HID_REPORT_TYPE_INPUT, data, len);
	}
#endif
	...
	for (i = 0; i < hid_svc.attr_count; i++) {
		if (hid_svc.attrs[i].read == read_report &&
			...
			int ret = bt_gatt_notify(conn, &hid_svc.attrs[i], data, len);

			if (ret) {
				LOG_WRN("bt_gatt_notify failed: %d", ret);
			}
			return ret;
		}
	}
...
int bt_hogp_device_request_mode(struct bt_conn *conn, uint8_t mode)
{
	...
	if (mode & BT_HOGP_MODE_ISO) {
		...
		ind_buf[0] = BT_HOGP_ISO_OPCODE_SELECT_HYBRID;
		...
		ind_buf[0] = BT_HOGP_ISO_OPCODE_SELECT_DEFAULT;
		len = 1;
	}
...
uint16_t bt_hogp_device_get_iso_interval_ms(struct bt_conn *conn)
{
	...
	if (!dc || !dc->iso_cis_connected || dc->iso_tx_interval_us == 0) {
		return 0;
	}

	/* Convert us to ms, minimum 1ms */
	uint16_t ms = dc->iso_tx_interval_us / USEC_PER_MSEC;
	return ms > 0 ? ms : 1;
}
#endif /* CONFIG_BT_HOGP_DEVICE_ISO */
