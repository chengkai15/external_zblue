...
static void iso_connected_cb(struct bt_iso_chan *chan)
{
	...
	if (bt_iso_chan_get_info(chan, &info) == 0) {
		...
		dc->iso_tx_interval_us = (uint32_t)info.iso_interval * 1250;
	} else {
		dc->iso_tx_interval_us = 1000; /* fallback 1ms */
	}
...
static void report_ccc_changed(const struct bt_gatt_attr *attr, uint16_t value)
{
	...
	LOG_WRN("report_ccc_changed: idx=%d value=0x%04x attr_handle=0x%04x",
	...
	if (value & BT_GATT_CCC_NOTIFY) {
		...
		LOG_WRN("report[%d] notify ENABLED, ntf_mask=0x%x",
		       idx, hid_svc.input_ntf_enabled);
	} else {
		hid_svc.input_ntf_enabled &= ~BIT(idx);
		LOG_WRN("report[%d] notify DISABLED, ntf_mask=0x%x",
		       idx, hid_svc.input_ntf_enabled);
	}
}
...
int bt_hogp_device_register(const struct bt_hogp_device_init_param *param)
{
	...
	if (sci_ctx.modes & HOGP_DEVICE_SCI_MODE_SUPPORTED) {
		...
		/*
		 * 0x20A3 (LE_Read_Min_Supported_Connection_Interval):
		 * Query controller for SCI presets if supported,
		 * otherwise hardcode presets for HOGP SCI Info characteristic.
		 */
		struct bt_dev *hdev = bt_dev_get(0);

		if (hdev && BT_CMD_TEST(hdev->supported_commands, 47, 3)) {
			/* TODO: issue 0x20A3 and populate sci_ctx.properties
			 * from response. For now fall through to hardcode.
			 */
		}

		LOG_WRN("SCI: hardcode SCI presets (0x20A3 not usable)");
		 *
		 ...
		 */
		{
			...
			*p++ = 3; *p++ = 0; *p++ = 90; *p++ = 0; *p++ = 1; *p++ = 0;
			...
			LOG_WRN("SCI: hardcoded %u bytes SCI Info properties",
			       sci_ctx.properties_len);
		}
	}
#endif /* CONFIG_BT_SHORTER_CONNECTION_INTERVALS */
...
uint16_t bt_hogp_device_get_iso_interval_ms(struct bt_conn *conn)
{
	...
	uint16_t ms = dc->iso_tx_interval_us / 1000;
	return ms > 0 ? ms : 1;
}
#endif /* CONFIG_BT_HOGP_DEVICE_ISO */
