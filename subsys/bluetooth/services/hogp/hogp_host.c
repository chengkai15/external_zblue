...
#define HIDS_DBG(...) LOG_DBG(__VA_ARGS__)
#define HIDS_ERR(...) LOG_ERR(__VA_ARGS__)
#define HIDS_WRN(...) LOG_WRN(__VA_ARGS__)
				     ...
				     struct bt_gatt_discover_params *params)
{
	...
	if (BT_UUID_16(params->uuid)->val == BT_UUID_GATT_CCC_VAL) {
		...
		HIDS_DBG("BAS[%u] CCC handle: 0x%04x", c->bat_idx, attr->handle);
	}
...
static void next_step(struct hogp_host_conn *c)
{
	...
	case HOGP_HOST_STATE_DISCOVER_BAS_DESCS: {
		...
		err = bt_gatt_discover(c->conn, &c->disc_params);
		if (err) {
			HIDS_WRN("BAS desc discovery failed: %d", err);
			if ((c->bat_idx + 1) < c->num_bats) {
				c->bat_idx++;
				next_step(c);
			} else {
	...
	case HOGP_HOST_STATE_READ_BAT_LEVEL: {
		...
		err = bt_gatt_read(c->conn, &c->read_params);
		if (err) {
			HIDS_WRN("BAS read level failed: %d", err);
			if ((c->bat_idx + 1) < c->num_bats) {
				c->bat_idx++;
				next_step(c);
			} else {
...
static int hogp_host_iso_connect(struct hogp_host_conn *c, uint16_t interval_us)
{
	...
	c->iso_tx_qos.rtn = 2;
#ifdef CONFIG_BES_ISO_USE_TEST_PARAMS
	/* BES Controller requires 0x2063 (Set_CIG_Parameters_Test):
	 * Host directly specifies ISO_Interval, NSE, BN, FT, Max_PDU
	 * so Controller does not need to derive them from SDU_Interval.
	 */
	c->iso_rx_qos.max_pdu = max_sdu_input;
	c->iso_rx_qos.burst_number = preset->nse;
	c->iso_tx_qos.max_pdu = max_sdu_output;
	c->iso_tx_qos.burst_number = preset->nse;
	c->iso_qos.num_subevents = preset->nse;
#endif
...
#ifdef CONFIG_BES_ISO_USE_TEST_PARAMS
	cig_param.c_to_p_ft      = preset->ft;
	cig_param.p_to_c_ft      = preset->ft;
	cig_param.iso_interval   = preset->iso_interval_us / 1250; /* 1.25ms units */
#else
	cig_param.c_to_p_latency  = (preset->iso_interval_us + 999) / 1000;
	cig_param.p_to_c_latency  = (preset->iso_interval_us + 999) / 1000;
#endif
